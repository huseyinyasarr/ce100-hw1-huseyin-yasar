    ...
    public class Sorting
    {
        ...
        **/
        ...
        public static int[] SelectionSort(int[] unsortedList)
        {
            ...
            {
                //minArray is now the current spot in the Main list
                minArray = MainIndex;
                ...
                {
                    //if the remaining element is less then the minArray we move the index to that spot.
                    //Will swap values after going through the entire list

                    //We are finding the next smallest element
                    if (unsortedList[RemainingIndex] < unsortedList[minArray])
                    {
                        //Update Minimum amount index
                        minArray = RemainingIndex;
                    }
                }

                //We now have the index of the lowest number in the remaining list. Swap it with the current i element in the list
                minValueFound = unsortedList[minArray];
                unsortedList[minArray] = unsortedList[MainIndex];
        ...
        **/
        ...
        /**
        *
        *	  @name   MasterMethod (mastermethod)
        *
        *	  @brief Master Method
        *
        *	  Master Method
        *
        *	  @param  [in] a [\b int]  inputArray
        *	  
        *	  @param  [in] b [\b int]  key
        *	  
        **/
        public static string MasterMethod(int a, int b, string fn)
        {
            double c = Math.Log(Convert.ToDouble(a), Convert.ToDouble(b));
            double d;
            try
            {
